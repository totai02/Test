 ...
 * @constructor
 */
function Theme(onComplelete, global) {
    this._onComplelete = onComplelete
...
Theme.prototype.loadImage = function (jsonPath) {
    ...
    if(typeof (GOWN.loader.resources[jsonPath]) === 'undefined') GOWN.loader.add(jsonPath).load(this.loadComplete.bind(this));
};
...
Theme.prototype.getScaleContainer = function (name, grid) {
    ...
    return function () {
        ...
        if (!texture) {
            throw new Error('The frameId "' + name + '" does not exist in the texture cache');
        }
...
Theme.prototype.getSpriteImage = function (name) {
    return new PIXI.Sprite(this.textureCache[name]);
};

Theme.prototype.getTextureCache = function (name) {
